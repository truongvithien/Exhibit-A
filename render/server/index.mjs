...
listen.listen(port).then(async port => {
  ...
  page.on('console', async message => {
    ...
    let fps = 30;
    if (message.text().includes('fps=')) {
        fps = parseInt(message.text().replace('fps=', ''));
    }

    let outputName = 'output';
    if (message.text().includes('outputName=')) {
        outputName = message.text().replace('outputName=', '');
    }
    ...
    if (message.text() === 'DONE') {
      ...
      const cmd = `ffmpeg -y -r ${fps} -f image2 -start_number 1 -i "${join(
        'screenshots',
        'frame-%04d.png'
      )}" -c:v libx264 -pix_fmt yuv420p -preset slow -crf 16 ${outputName}.mp4`
